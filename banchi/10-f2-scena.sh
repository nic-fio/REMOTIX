...
#
# ⭐ E si guarda l'immagine, non il conteggio: `10-f1-testimone.py` scrive i
...
#            PORTA (def. 8420)
# ===========================================================================
# ⛔⛔ IL DOPPIONE E' STATO SANATO — 25 agosto 2026.
#
#    Questo banco chiamava un `10-f2-testimone.sh` suo, che faceva **la stessa
#    cosa per la stessa strada** di `banchi/10-f1-testimone.py`: il cliente di prova
#    con `--video-scrivi`, poi `ffmpeg`.  Due incarichi paralleli l'hanno
#    scritto tutt'e due, senza saperlo.
#
# ⭐ Si e' tenuto quello di F1, e la ragione e' una sola: **e' certificato**
#   (`--certifica`, 4 predicati, sano 4 → guasto 12 → risanato 4) e distingue
#   i **tre esiti** — «si vede», «e' nero», ⛔ **«non ho guardato»**.
#
# ⭐ E la sola cosa che l'altro sapeva fare in piu' e' stata **portata dentro**
#   quello che resta: `--tutti` tira giu' TUTTI i fotogrammi della presa.
#   ⛔ Non e' un lusso — `[M]` il dialogo «Profile Missing» compariva a meta'
#   della presa e sull'ultimo fotogramma **non c'era piu'**.
set -uo pipefail
