...
INNESTI = [
    ...
    (
        ...
        "\t\treturn;\n"
        "\t}\n"
        "\t/* \u26d4\u2b50 E LO STESSO VALE PER `respinto-non-riprovare` — rilievo\n"
        "\t *   R12-A.47, 11 agosto 2026, e la ragione e' scritta due righe\n"
        "\t *   sopra: la chiusura parte subito dietro al RESPINTO e CORRE\n"
        "\t *   contro il lettore della pagina.\n"
        "\t *\n"
        "\t *   `[M]` 11 agosto, Firefox 140.13.0esr: la pagina non arrivava\n"
        "\t *   nemmeno al ramo del RESPINTO — vedeva lo stream cadere e\n"
        "\t *   dichiarava `canale-rotto` dove l'atteso dice `muta`.  \u26a0 E la\n"
        "\t *   pagina aveva RAGIONE: distinguere un FIN da una caduta e' la\n"
        "\t *   cura del rilievo R6.12, e qui il server non manda nessun FIN —\n"
        "\t *   chiude la SESSIONE con CLOSE_WEBTRANSPORT_SESSION.\n"
        "\t *\n"
        "\t * \u2b50 Quel che questo caso vuole vedere e' che dopo `RESPINTO` la\n"
        "\t *    pagina NON RIPROVI (\u00a74.4).  Per vederlo bisogna lasciarle il\n"
        "\t *    tempo di non farlo: si tace, e chi chiude sara' lei.\n"
        "\t * \u26d4 Non e' allargare l'atteso: l'atteso resta `muta`, e il caso\n"
        "\t *    puo' ancora dire di no — se la pagina riprovasse, i byte in\n"
        "\t *    piu' li vedrebbe il registro del SERVER, che e' il testimone\n"
        "\t *    che questo caso dichiara da sempre (\u00a78.1). */\n"
