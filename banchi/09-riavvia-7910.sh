#!/bin/sh
# 09-riavvia-7910.sh — IL SERVER DEL **DOPO**, sulla porta 7910.
#
# ⛔⭐ PERCHE' ESISTE, e non e' un doppione di `src/riavvia-7900.sh`: il 23
#    agosto 2026 sono state applicate tre cure al codice, e per giudicarle
#    serve il confronto APPAIATO — il **prima** (7900, binario di stamattina,
#    albero `09-src`) vivo e intatto accanto al **dopo** (7910, albero
#    `09b-src`).  ⚠ Un solo server ricostruito sul posto avrebbe cancellato il
#    termine di paragone, e nessuna misura di ieri sarebbe piu' ripetibile.
#
# ⛔ E i due NON si misurano insieme (`LEZIONI.md` §1.26): uno per volta.
...
#
#      porta       7910          (7900 = il PRIMA, 7730 = l'utente, 7790 = fase 8)
#      lavoro      /media/REMOTIX/tmp/09b
#      albero      /media/REMOTIX/src/09b-src/src
#      unita'      remotix-7910.service
#
...
#
# Riavvia il server di prova sulla porta 7910 con la pagina che c'e' adesso su
...
#
# ---------------------------------------------------------------------------
# ⛔⛔ QUESTO FILE STA NEL DEPOSITO DAL PRIMO MINUTO — e il gemello no.
#
...
#         `user@` ne' `session-`.
set -e
LAV=/media/REMOTIX/tmp/09b
SRC=/media/REMOTIX/src/09b-src/src
...
#   subito dopo si e' preso «⛔ non mi lego a 0.0.0.0:7910 in UDP: Address
#   already in use» e il server nuovo e' morto senza che nessuno guardasse.
i=0
while ss -uln 2>/dev/null | grep -q ':7910 ' && [ $i -lt 50 ]; do i=$((i+1)); sleep 0.2; done
...
systemd-run \
  ...
  "$SRC/remotix" \
  --indirizzo 0.0.0.0 --nome 192.168.0.2 --porta 7910 \
