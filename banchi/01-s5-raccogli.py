#!/usr/bin/env python3
"""01-s5-raccogli.py — serve la pagina di S5 e ne REGISTRA quel che vede.

    python3 -u 01-s5-raccogli.py [porta]        predefinita: 8866
...
⛔ E QUI IL REGISTRO E' L'UNICO OCCHIO CHE ABBIAMO.  Il browser gira su uno
   schermo finto: nessuno puo' *leggere* i numeri sulla pagina.  Quel che la
   pagina non spedisce non e' successo per nessuno.
...
class Raccoglitore(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        print(f"=== {dati['ora']}  {dati.get('tipo')}  "
              f"schermo={dati.get('schermo_l')}x{dati.get('schermo_a')}  "
              f"dpr={dati.get('dpr')}  tela={dati.get('tela_l')}x{dati.get('tela_a')}", flush=True)
...
if __name__ == "__main__":
    porta = int(sys.argv[1]) if len(sys.argv) > 1 else 8866
    print(f"== S5: pagina su http://127.0.0.1:{porta}/01-s5-pagina.html")
