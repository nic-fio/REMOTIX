# R2 — Revisione avversariale di `STUDI.md` §web

*9 agosto 2026. Revisione della sintesi `STUDI.md` §web (330 righe) contro le quattro fonti:
`S1-certificato.md` (920), `S2-decodifica.md` (732), `S3-tastiera-appunti.md` (1.391),
`S4-ritardo-disegno.md` (671).*

**Che cosa ho fatto.** Ho riportato alla fonte **96 affermazioni** di `STUDI.md` §web: ogni citazione
...
`DECISIONI.md` §1.7 sono **già stati riscritti citando `STUDI.md` §web**. Tre dei rilievi qui sotto sono
...
```
DOVE:             STUDI.md §web §3.3, riquadro «Il banco», e §7 riga S1a
COSA CONTRADDICE: S1 §4.2 e §4.4; LEZIONI.md §1.9 regole 1 e 2; REVIEWER.md §1
                  punti 4 e 5
```

**Come si dimostra.** `STUDI.md` §web §3.3 scrive: *«Il controllo positivo è ovvio e va fatto lo stesso:
...
Il caso concreto che rompe il controllo di `STUDI.md` §web: **il firewall del server blocca UDP 7447**.
Allora P1 fallisce su Safari *e* fallisce su Chrome — cioè il controllo di `STUDI.md` §web è **verde** — e
...
⛔ **E il costo è massimo perché è la prima misura**: `STUDI.md` §web §3.3 la dichiara *«la prima misura del
...
### R2 — «Safari non ha `serverCertificateHashes`» è l'errore che S1 corregge per nome, e `STUDI.md` §web non riporta la correzione

```
DOVE:             STUDI.md §web §3.1 (riga Safari), §3.2 (riga «la strada»), §7 riga S1a,
                  §8 (prima riga)
...
`STUDI.md` §web **non riporta mai** che Safari abbia `serverCertificateHashes`. La riga Safari di §3.1
...
1. `STUDI.md` §web §7 motiva S1a così: *«decide se iPhone e iPad hanno **una strada senza dominio**»*.
   ...
   **sempre** […]; se poi Safari accettasse anche senza, sarebbe **un ripiego in più**, non un
   ...
   cade la ragione per cui `STUDI.md` §web la mette per prima davanti a tutto;
...
```
DOVE:             STUDI.md §web §6.1, riquadro «⚠ E impone la forma della pagina»
COSA CONTRADDICE: S3 §3.A.4 (chiusura) e §3.A.6; S4 §5.1 regola 6
```

**Come si dimostra.** `STUDI.md` §web §6.1 chiude la forma della pagina in due mestieri e due thread, e
...
### R4 — «Nessuna [misura] richiede una riga di prodotto» è smentito da due rapporti su quattro, e dallo stesso `STUDI.md` §web

```
DOVE:             STUDI.md §web §7, riga d'intestazione del piano delle misure
COSA CONTRADDICE: S4 §5.3 punto 5 e §4.1; S2 §5 esito C; e STUDI.md §web §1.2 C e §4.3
```
...
E `STUDI.md` §web si contraddice da solo a tre sezioni di distanza: §1.2 C — *«la diagnosi non può stare in
...
```
DOVE:             STUDI.md §web §1.2 A, riga «S2» della tabella
COSA CONTRADDICE: S2 §3.7 e §7 («Non l'ho trovato»); LEZIONI.md §2.3-quater;
                  REVIEWER.md §2 forma E5
```

**Come si dimostra.** `STUDI.md` §web scrive: *«sui fotogrammi decodificati in hardware `VideoFrame.format`
...
⛔ **Perché costa**: quella riga è **una delle tre** su cui `STUDI.md` §web §1.2 A costruisce la richiesta
...
marca. (La stessa `LEZIONI.md` §2.3-quater che `STUDI.md` §web cita a sostegno è la lezione che vieta
questo.)
...
### R6 — «Compra la tastiera intera» è più forte di quel che S3 sostiene, e `STUDI.md` §web si smentisce da solo trenta righe dopo

```
DOVE:             STUDI.md §web §1.2 B (conclusione in grassetto)
COSA CONTRADDICE: S3 §2.1, §2.2, §2.4 (tabella dei sistemi), §5.1-bis, §5.5;
                  e STUDI.md §web §5.2 e §5.5
```
...
4. ⛔ **e sull'uso primario è una `[?]`.** S3 §5.1-bis: *«`[?]` **resta da misurare se valga anche
   per Chrome per Android**»*. Lo dice anche `STUDI.md` §web §5.5 — *«le due `[?]` che contano di più […]
...
per parola, citando `STUDI.md` §web §1.2 B.
...
```
DOVE:             STUDI.md §web §6.2, riquadro «⭐ La leva, se servisse»
COSA CONTRADDICE: S2 §3.10 (riga moonlight-web); S4 §3.9 e §6; S4 §3.4
```

**Come si dimostra.** `STUDI.md` §web scrive: *«Selkies e **moonlight-web** non dipingono su canvas —
...
|---|---|---|
| **moonlight-web** (`linckosz`) — citato da **S2**, e da `STUDI.md` §web stesso in §4.2 | S2 §3.10 | ⛔ *«il video **non** va in `<video>`: WebCodecs in un worker → **WebGPU**»* |
| **moonlight-web-stream** (`MrCreativ3001`) — citato da **S4** | S4 §3.9, §6 | è questo che usa `MediaStreamTrackGenerator` → `<video>` |

Quindi `STUDI.md` §web fa dire a `moonlight-web` — lo stesso progetto che cita due sezioni prima per
...
quantifichi**»*. `STUDI.md` §web presenta una leva di latenza `[R]` dove S4 ha un'ipotesi `[?]` su un
...
`STUDI.md` §web fonde A e A′ in una riga sola e attribuisce alla combinazione l'universalità che vale solo
...
```
DOVE:             STUDI.md §web §3.3 («`[?]` La seconda: […] Il rapporto dice circa sette giorni —
...
`STUDI.md` §web §8 la mette invece fra *«quel che questo studio NON sa»*. È il rovescio di E5: una cosa
...
Cioè: **anche con il certificato della pagina longevo e stabile — la cura che `STUDI.md` §web §3.2
prescrive — su Chrome l'avviso torna ogni sette giorni.** `STUDI.md` §web §3.2 dice solo che *«confonderli
...
```
DOVE:             STUDI.md §web §6.2 e §1.1 riga 5
...
`STUDI.md` §web ha saldato la **frazione** di un punto del rapporto al **totale** di un altro, e ne esce
una riga che non torna con l'aritmetica. Chi la userà per dimensionare il tetto sbaglierà di 9 ms
sull'estremo basso — su un tetto di 50, e su un numero che `STUDI.md` §web chiede di scrivere accanto al
...
```
DOVE:             STUDI.md §web §2, riga «WebCodecs» della mappa
...
`STUDI.md` §web, che altrove tratta Firefox come uno dei tre motori serviti.
...
```
DOVE:             STUDI.md §web §5.1, riga «non è più solo Chrome»
...
`STUDI.md` §web la scrive come un fatto, dentro una riga marcata `[S]`.
...
lezione che `STUDI.md` §web §9.1 celebra a parole nella sezione delle lezioni, e omette dove serviva.
...
Il caso concreto: si copia `dom_code_data.inc` come dice `STUDI.md` §web, un utente Firefox preme il tasto
...
`STUDI.md` §web §5.4 dice invece che la conversione è **«già scritta e verificabile»**: una frase vera in
...
```
DOVE:             STUDI.md §web §4.2, riga «il formato del flusso» e la chiusura
...
La conclusione (Annex-B) è la stessa; **la ragione che `STUDI.md` §web le mette accanto è più forte del
vero**, e su Safari è falsa. Un rapporto onesto sul costo per fotogramma su iPhone partirebbe da
qui, e `STUDI.md` §web lo toglie.
...
*«Basta **non** impostare `AV_CODEC_FLAG_GLOBAL_HEADER`»*, che `STUDI.md` §web non riporta. Nella riga di
`STUDI.md` §web finisce sotto lo stesso `[R]` degli altri due fatti.
...
```
DOVE:             STUDI.md §web §4.3, riga «⛔ il controllo positivo»
COSA CONTRADDICE: S2 §4.4, controlli A, B e il «criterio di validità»;
                  REVIEWER.md §1 punto 5
```

**Come si dimostra.** `STUDI.md` §web riporta il solo **controllo A** (VP9 forzato in software). S2 §4.4 ne
...
```
DOVE:             STUDI.md §web §6.3, riga «⛔ il controllo decisivo»
COSA CONTRADDICE: S4 §4.2, controlli P3 e P5; LEZIONI.md §1.9 regola 1
```

**Come si dimostra.** `STUDI.md` §web riporta il solo **P1** (il ritardo iniettato). S4 §4.2 ne elenca sette,
...
```
DOVE:             STUDI.md §web §1.2 C e §4.3 (riquadro finale)
...
`STUDI.md` §web la promuove a conclusione generale (*«Quindi la diagnosi non può stare in un banco di
...
| **O6** | **S1 §5 punto 3** — la rotazione | *«la pagina già aperta in un browser ha in mano **un'impronta che invecchia**: alla riconnessione dopo la rotazione va ricaricata la pagina o richiesta l'impronta corrente. **Va deciso dove sta questo aggiornamento in RCP**»*. `STUDI.md` §web §3.2 dice solo *«ruota da sé»*, che è la metà facile |
| **O7** | **S3 §3.A.2** — i bottoni a schermo | Tre riferimenti maturi su tre danno all'utente pulsanti/tastiera a schermo per quel che il browser non lascia passare (`Ctrl+Alt+Canc`); S3: *«⭐ **è un requisito, non un ripiego di fortuna»***. `STUDI.md` §web §5.2 dichiara `Ctrl+Alt+Canc` «non recuperabile» e si ferma lì |
| **O8** | **S3 §3.A.2** — lo **stato B** | Tre stati, non due: *«**B — consegnata ma riservata** […] ⛔ **il peggiore**: la sessione remota riceve la battuta **e** la scheda si chiude»*, e su Firefox `Ctrl+Tab` è lì (*«sembra intercettabile e non lo è»*). S3 §5.5 chiede per questo di **riformulare la misura S3 di `PIANO.md` §1.2**. `STUDI.md` §web non nomina né gli stati né la riformulazione, e §7 riga S3a chiede ancora «la lock su DeX» come se la domanda fosse una sola |
| **O9** | **S3 §3.B.6** — iPhone | *«lo schermo intero su iPhone: supporto **parziale** in tutte le versioni da 12 a 26.5 `[S]`. ⛔ Senza schermo intero **non c'è keyboard lock**, quindi su iPhone **si perde tutto il §3.A insieme»***. `STUDI.md` §web mette iPhone al primo posto del piano delle misure (§7 S1a) senza dire che su quel dispositivo la tastiera è persa comunque |
...
| **O11** | **S4 §5.1 regola 7** | Il cross-origin isolation (COOP+COEP) è in S4 una **regola di prodotto** (*«la pagina si serve cross-origin isolated»*), con conseguenze su come si servono le risorse. `STUDI.md` §web §6.3 lo degrada a taratura del righello del banco (*«il righello va tarato»*) |
...
citazione inventata in `STUDI.md` §web. In particolare, verificate parola per parola alla fonte:

| `STUDI.md` §web | Torna in |
|---|---|
...
`STUDI.md` §web §8 la elenca sotto *«ogni rapporto ha la sua lista»*, e questa è di `RCP.md`);
...
riga di `STUDI.md` §web §9.1 è la più solida del documento.

⭐ **E una cosa che `STUDI.md` §web fa meglio delle fonti**: dichiarare in testa che `[M]` non compare mai,
...
1. **Ho cercato citazioni `[R]` inventate** — file, riga o funzione che nel rapporto non ci fossero,
   o che dicessero altro. **Non ho trovato niente**: `STUDI.md` §web porta **28 marche `[R]`**, le ho
   controllate una per una, e tutte esistono nella fonte con il senso che `STUDI.md` §web gli dà. I difetti
...
4. **Ho provato a smentire «l'Annex-B è la strada giusta»** cercando in S2 una ragione per l'hvcC.
   **Non ho trovato niente**: le tre ragioni di S2 §3.5 reggono, il costo su WebKit c'è ma è
   ...
   conclusione di `STUDI.md` §web §4.2 è giusta; è la sua *motivazione* a essere più forte del vero (R14).
5. **Ho provato a trovare in `STUDI.md` §web un `[R]` costruito su una misura di terzi** spacciata per
   ...
   **non compaiono in `STUDI.md` §web in nessuna forma**. La sintesi ha ereditato la prudenza della fonte.
6. **Ho provato a trovare una degradazione silenziosa proposta** (invariante I1) fra le conseguenze
   di prodotto di `STUDI.md` §web. **Non ho trovato niente**: le due volte in cui propone un ripiego (§6.2
...
7. **Ho controllato i conteggi delle righe dei rapporti** dichiarati in testa a `STUDI.md` §web: S1 = 920
...
|---|---|
| **R1, R15, R16** — i tre banchi | ⛔ **prima di scrivere una riga di banco.** Sono correzioni di testo nei rapporti e in `STUDI.md` §web, costo zero, e senza di esse la fase 1 misura male |
...
| **R5, R6** | correggere le marche in `STUDI.md` §web, **e nei due documenti dove sono già passate** (`SPECIFICHE.md` §7.3-bis, `DECISIONI.md` §1.7) |
| **R7-R14, R17** | correzioni di testo in `STUDI.md` §web |
