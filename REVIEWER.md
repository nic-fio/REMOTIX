...
> stanno sotto `fondamenta/documenti/`. In V2 si legge **`SPECIFICHE.md`** al loro posto, e chi revisiona
...
| ⭐⭐ **E14** | **Il banco tace invece di dare rosso** | ⛔ **La forma più frequente in assoluto**: `[M]` fase 9, **nove difetti di banco su nove**. Un predicato che non può fallire, una funzione che torna **0 invece di `None`** quando non ha potuto leggere, una guardia messa dove il numero **si produce** invece di dove **si consuma**, una parola cercata dentro un testo che la contiene per un altro motivo. ⛔ Il segnale che la smaschera: **quel predicato è mai stato visto fallire?** Se non c'è il caso rosso in `--certifica`, **e fatto girare**, il banco non è finito. | `LEZIONI.md` §1.29 |
| ⭐⭐ **E15** | **La grandezza sbagliata, che ordina i casi al contrario** | Non una soglia da ritarare: una **grandezza** che misura un'altra cosa. `[M]` Fase 9: `pkt_lost/pkt_sent` su una linea che **riordina** misura **il riordino**, e una linea che **regge** ne dichiarava il **512‰** contro il **123‰** di una che **non regge** ⇒ nessuna soglia poteva separarle. ⛔ Il segnale: **prima di tarare, prova la grandezza sui due estremi noti** e verifica che li **ordini** nel verso giusto. Se non li ordina, ritararla è tempo perso. | `LEZIONI.md` §1.33, `fasi/09` §18.1 |
