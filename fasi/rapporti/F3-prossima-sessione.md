...
| **B6** | ⭐ **l'occasione dentro l'occasione** | `EncSliceLP` è l'entrypoint che `STUDI.md` §web nomina come *«da verificare»* per i **sotto-livelli temporali**: la strada per **abbandonare un fotogramma senza rompere quelli dopo**, che oggi costa **una chiave ogni volta**. ⚠ **Non farla dentro questa corsia**: si nomina, si misura la fattibilità, e diventa lavoro suo |
...
| # | quando | | |
|---|---|---|---|
| **K1** | ✅ **FATTO — e CHIUSO NEI FATTI la sera del 13** | **IL PUNTO CIECO** | ⭐ Verificato e detto più preciso di com'era scritto: `src/codificatore.c` non compariva in **nessuna** lista — vero; ma *«e nemmeno `figlio.c`»* era **smentito** (`figlio.c` c'era, nella lista di `03-b17`), solo che **`03-b17` non era mai stato provato** ⇒ la rete c'era **sulla carta**. ⭐⭐ **Adesso c'è nei fatti**: `codificatore.c` e `.h` sono a catalogo su `03-b17` e `03-b19`, e ⭐ **`03-b17` è CERTIFICATO** (38/38 → 37/38 → 38/38, marca misurata) ⇒ `--punti-ciechi` lo dà **coperto**, non «sulla carta». ⛔ **Ma il punto cieco vero era molto più grande, e nessuno strumento sapeva dirlo**: `--punti-ciechi`, scritto la sera stessa, conta **22 file del prodotto che nessuna voce del catalogo nomina** — `cattura.c`, `mutter.c`, `sessione.c`, `tls.c`, `trasporto.c`, `registro.c`, `comando.c`… ⇒ **K1 è chiuso; la domanda che K1 ha aperto no** |
...
| | |
|---|---|
| ⛔⛔⛔ **~~tre~~ CINQUE banchi che escono SEMPRE 0** — il conto è stato corretto **due volte nella stessa sera, da due agenti diversi, e in tutt'e due i casi il banco era di chi contava**: `ko()` **stampa e basta**, e il `return` finale è incondizionato ⇒ col guasto dentro il rosso resta **nella prosa** e chi legge a macchina vede verde. ⭐ Due curati l'11 agosto · ⛔ **il quarto era `03-b17-ritardo.py --verdetto`, cioè IL BANCO DEL NUMERO DELLA FASE** (trovato e curato dalla corsia C, misurato nei due versi su un verbale vero: prima 0, dopo 1) · ⛔ **il quinto era `03-palco-dipinge.py`, scritto dal COORDINATORE la sera stessa in cui questa voce veniva catalogata** (trovato dall'agente B0, **fuori dal suo perimetro**, e detto invece che lasciato correre) · ⏳ resta `03-b19-ritardo-worker.py`. ⇒ **Non è una svista che si ripete: è una forma che il linguaggio invita**, e va cercata in ogni banco nuovo prima di crederci |
