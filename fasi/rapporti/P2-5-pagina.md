...
| ⚠ **il ritardo** | questo anello misura **che** il pixel arrivi, non **quando**. La tela desincronizzata di `web.md` §6.1 è **fuori** da questo banco, perché su una tela desincronizzata `getImageData` legge da un buffer che il compositore può avere già scambiato — cioè rende ambigua la rilettura su cui poggia il metro della fase (I8). ⛔ *Questa riga diceva che stava «dietro un interruttore **spento** (`?tela=desincronizzata`)», ed è **falso**: `[M]` 13 agosto 2026, `src/pagina.c:243` fa cadere in **404** qualunque percorso con un `?` ⇒ ⛔ **quell'interruttore non è mai stato raggiungibile.** Non è un interruttore spento: è un interruttore **che non c'è**. Non visto da nessuno perché i banchi servono la pagina da un `http.server` di Python, che il `?` lo ignora* |
