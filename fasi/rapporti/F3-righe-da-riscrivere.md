...
| `LEZIONI.md` (§1.x nuova) | ⭐ **su Xvfb `requestAnimationFrame` non gira mai** — `[M]` 0 quadri in 3 s, con e senza GPU, `visibilityState` «visible». Ogni cammino di prodotto che ci passa dietro è **codice morto sul banco**, e vale per TUTTI i banchi browser del progetto | da fare |
