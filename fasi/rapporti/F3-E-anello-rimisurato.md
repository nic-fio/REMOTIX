...
*13 agosto 2026, notte. ⛔ **Il numero della fase NON è stato prodotto**, e la ragione non è che è
mancato il tempo: è che **misurarlo stanotte avrebbe prodotto un numero falso**, e la corsia E
esiste per sottrarne due, non per consegnarne uno qualsiasi.*
...
| # | dove | che cosa |
|---|---|---|
| **1** | ⛔⛔ `src/pagina.html` | **la sessione vera non dipinge HEVC e la tela crolla a 320×240.** Tre cose da guardare: (a) HEVC dà `EncodingError` **con la GPU vera**; (b) viene **offerto lo stesso**, quindi `codec_buoni` non l'ha filtrato; (c) il suo fallimento **trascina `video.misura_massima`** anche se AV1 arriva a 4K. ⇒ Finché non è chiuso, **nessun banco che passi dalla sessione può esercitare il codificatore HEVC in hardware** |
...
| | |
|---|---|
| ⛔⛔ **il numero della fase 3 con la codifica in hardware** | `[?]` **intero**: non è stato possibile farci arrivare un fotogramma |
...
| ⚠ **E4 — quanto vale il tratto della codifica in hardware** | `[?]`: sul cammino seriale del prodotto non è stato possibile misurarlo. ⭐ Quel che si sa: il tratto 2 (codifica + filo) vale **39,8 ms**, e il filo ne vale **0,26** ⇒ ⭐ **la codifica è ~39,6 ms, il 55 % del totale** — il bersaglio è quello, e il numero del 13 agosto regge |
